...
--QUERY 8
SELECT vendor.vendor_id, vendor_name, COUNT(vendor_booth_assignments.vendor_id) AS booth_count
FROM vendor
INNER JOIN vendor_booth_assignments
ON vendor.vendor_id = vendor_booth_assignments.vendor_id
GROUP BY vendor.vendor_id, vendor_name;
...
--QUERY 9
SELECT customer.customer_id, customer_first_name, customer_last_name, SUM(quantity * cost_to_customer_per_qty) AS total_spent
FROM customer
INNER JOIN customer_purchases
ON CUSTOMER.customer_id = customer_purchases.customer_id
GROUP BY customer.customer_id, customer_first_name, customer_last_name
...
*/
--QUERY 10
SELECT * INTO temp.new_vendor
FROM vendor;
SELECT * FROM temp.new_vendor;
VALUES(10, 'Thomass Superfood Store', 'Fresh Focused', 'Thomas Rosenthal', '2024-01-01');
S
